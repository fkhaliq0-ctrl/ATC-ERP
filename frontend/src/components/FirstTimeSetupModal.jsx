...
const FirstTimeSetupModal = ({ isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('agentPhone');
    if (saved) {
      setPhone(saved);
    }
  }, [isOpen]);
      ...
      <div className="setup-modal-card">
        ...
        <form onSubmit={handleSubmit} className="setup-modal-body">
          ...
          <button type="submit" className="btn-setup-submit" disabled={isVerifying}>
            {isVerifying ? 'Verifying on WhatsApp...' : '📱 Save & Verify on WhatsApp'}
          </button>
        </form>
