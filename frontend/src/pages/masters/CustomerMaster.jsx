...
const CustomerMaster = () => {
  ...
  const generateNextCustNo = (custList) => {
    const maxNum = custList.reduce((max, c) => {
      const num = parseInt((c.customer_no || '').replace('CUST-', ''), 10);
      return isNaN(num) ? max : Math.max(max, num);
    }, 0);
    setCustomerNo(`CUST-${String(maxNum + 1).padStart(4, '0')}`);
  };
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setCustomers(data);
      generateNextCustNo(data);
    } catch (error) {
      console.error('Error fetching customers:', error);
      setCustomerNo('CUST-0001');
    }
  };
  ...
  const handleEdit = (customer) => {
    ...
    document.getElementById('idNumber').value = customer.gstin || customer.pan || '';
  };
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#1a237e', margin: 0 }}>👤 Customer Master</h1>
            <p style={{ color: '#666', fontSize: '14px', margin: '4px 0 0 0' }}>Add and manage your customers</p>
          </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr 1.8fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#333', marginBottom: '4px' }}>Customer No.</label>
                <input value={customerNo} disabled style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '13px', backgroundColor: '#f0f0f0', color: '#666', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#333', marginBottom: '4px' }}>Customer / Company Name *</label>
                <input id="customerName" placeholder="Company or Individual Name" style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }} required />
              </div>
              <div>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#333', marginBottom: '4px' }}>Customer Address *</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.5fr', gap: '12px', marginBottom: '12px' }}>
              ...
              <div>
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="customer@example.com" style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', borderTop: '1px solid #e0e0e0', paddingTop: '12px' }}>
              <button type="submit" disabled={loading} style={{ padding: '10px 24px', backgroundColor: '#1a237e', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer' }}>
                {loading ? '⏳ Saving...' : editingId ? '✏️ Update' : '💾 Save Customer Master'}
              </button>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#1a237e', marginBottom: '12px', borderBottom: '2px solid #e8eaf6', paddingBottom: '12px' }}>📋 Saved Customers ({customers.length})</h3>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #e0e0e0' }}>
                  <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 'bold', color: '#555', fontSize: '12px' }}>Cust ID</th>
              ...
              <tbody>
                {customers.length === 0 ? (
                  <tr><td colSpan={isSuperUser ? 7 : 6} style={{ textAlign: 'center', padding: '24px', color: '#888' }}>No customers recorded yet.</td></tr>
                ) : (
                  customers.map((cust, index) => (
                    <tr key={cust.id || index} style={{ borderBottom: '1px solid #f0f0f0', backgroundColor: index % 2 === 0 ? 'white' : '#fafafa' }}>
                      <td style={{ padding: '10px 12px' }}>{cust.customer_no || `CUST-${String(index + 1).padStart(4, '0')}`}</td>
                      <td style={{ padding: '10px 12px' }}>{cust.customer_name}</td>
                      <td style={{ padding: '10px 12px' }}>{cust.contact_person || '-'}</td>
                      <td style={{ padding: '10px 12px' }}>{cust.city || '-'}</td>
                      <td style={{ padding: '10px 12px' }}>{cust.phone || '-'}</td>
                      <td style={{ padding: '10px 12px' }}>{cust.gstin || '-'}</td>
                      {isSuperUser && (
                        <td style={{ padding: '10px 12px' }}>
                          <button onClick={() => handleEdit(cust)} style={{ padding: '4px 12px', backgroundColor: '#1976d2', color: 'white', border: 'none', borderRadius: '4px', fontSize: '11px', cursor: 'pointer', marginRight: '6px' }}>✏️ Edit</button>
                          <button onClick={() => handleDelete(cust.id)} style={{ padding: '4px 12px', backgroundColor: '#d32f2f', color: 'white', border: 'none', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>🗑️ Delete</button>
                        </td>
                      )}
                    </tr>
                  ))
